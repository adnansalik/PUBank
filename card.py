...
def luhn(number):
    ac = list(number)
    ac = [int(x) for x in ac]  # Converts the digits of the AccountNumber to <class 'int'>

    for i in range(0, len(ac), 2):  # Multiplies the digits at odd(here even) index by 2
        ac[i] = ac[i] * 2

    for i in range(len(ac)):  # Subtracts the digits of the AccountNumber by 9 if they are greater than 9
        if ac[i] > 9:
            ac[i] = ac[i] - 9
...
def create_card():
    ...
    while True:
        ...
        try:
            db.do_entry(account, name, card_number, card_pin, 0)
